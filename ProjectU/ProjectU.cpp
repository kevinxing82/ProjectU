	...
	_In_ int       nCmdShow)
{
	...
	LoadStringW(hInstance, IDC_PROJECTU, szWindowClass, MAX_LOADSTRING);
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_PROJECTU));
	wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_PROJECTU);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	AdjustWindowRectEx(&rect, WS_OVERLAPPEDWINDOW, GetMenu(hWnd) != NULL, 0);
	float offsetX = (srcWidth - rect.right) / 2;
	float offsetY = (srcHeight - rect.bottom) / 2;
	MoveWindow(hWnd, offsetX, offsetY, rect.right - rect.left, rect.bottom - rect.top, false);
